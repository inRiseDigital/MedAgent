...
- [x] **Compliance pack assembled** — [COMPLIANCE-PACK.md](COMPLIANCE-PACK.md) (artifact index + DPIA-lite: data categories, lawful basis/consent, flows + cross-border minimisation, retention, subject rights, go/no-go checklist, sign-off table), [THREAT-MODEL.md](THREAT-MODEL.md) (STRIDE mapped to built/deferred mitigations + ranked residual risks), consent export + audit report capabilities are the shipped FR-5.6/5.8 features. R-2 dep-audit now gated in CI. (DPIA sign-off + R-1/R-3 remain for go-live.)
- [x] **Observability — metrics live end-to-end** · `configure_telemetry` was a no-op stub; implemented it in all 3 services (prometheus-fastapi-instrumentator → /metrics with the exact dashboard metric names) + enabled Traefik prometheus metrics (:8082). Verified: `--profile observability up` → prometheus targets core-api/agent-service/notify-service/gateway all UP, live per-service `http_requests_total`, Grafana "Golden Signals" dashboard + datasources provisioned (see [RUNBOOKS.md](RUNBOOKS.md) §6). (OTLP traces/logs to Tempo/Loki + alert rules remain; metrics + dashboard are live.)
