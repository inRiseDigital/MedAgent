...
- [x] **5.3 Telemedicine + guardian/family join.** `POST /telemedicine/sessions` provisions a virtual Appointment + video room and issues **single-use** join tokens (Redis `GETDEL`, the SSE-ticket pattern) for the patient, clinician, and every guardian resolved via `RelatedPerson` (the 4.1 proxy); `GET .../join?token=` redeems one; `start` opens a virtual Encounter, `complete` finishes it; the post-consult e-Rx reuses the normal screened `POST /proposals/commit` referencing the Encounter. *Verified:* guardian (Nimal) joins Baby Perera's consult as `guardian`; reused token → 401; start→complete lifecycle + double-start 409; e-Rx after → screened MedicationRequest (verdict pass) linked to the encounter; audit chain intact throughout.

## Track 6 — Intelligence (last)
- [x] **6.1 Imaging AI triage** (deterministic-first). `POST /imaging/{order}/report` attaches an `ImagingStudy` + `DiagnosticReport` to an imaging order and triages it with a curated ruleset (normal/abnormal/urgent) — the AI proposes findings, the ruleset sets the flag + routing; low AI confidence (<0.5) forces `needs_review`. Urgent findings auto-raise a `Task` referral into the receiving specialty's inbox (closed loop with 5.1). `GET /imaging/reports?patient=` lists reports urgent-first. Audited (imaging_reported, imaging_urgent_flag). *Verified:* clear CXR → normal/no referral; tension pneumothorax → urgent + auto-referral to Respiratory in the teaching-hospital inbox; low-confidence nodule → abnormal + needs_review; audit chain intact (18 events). (Real PACS/DICOM + live AI model wrap this contract in prod.)
