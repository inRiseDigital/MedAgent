"""Keycloak JWT authentication (02) — shared across all Python services.

Validates Bearer access tokens against the realm's JWKS (fetched from the
internal certs endpoint, cached with a TTL). Realm roles are extracted from
`realm_access.roles`.

Notes:
- AUTH_DISABLED=true short-circuits to a static dev principal — local
  bootstrapping only, never set outside local dev.
- Audience (`aud:<service>`) enforcement is a TODO for when the realm-as-code
  client scopes land (02 §2); issuer, signature and expiry ARE verified now.
- Clock-skew leeway is read from the caller's Settings if present
  (`jwt_leeway_seconds`), else 0 — so every service shares one implementation.
"""
...
async def require_user(
    ...
    try:
        ...
        claims = jwt.decode(
            ...
            leeway=getattr(settings, "jwt_leeway_seconds", 0),  # tolerate small clock skew (RFC 7519)
            # TODO(S1/S2): enforce audience `aud:<service>` once the realm client
