...
- order_lab(test) → when the doctor asks to ORDER/REQUEST a lab test: stages a Confirm card; the lab order (FHIR ServiceRequest) is placed only when the doctor taps Confirm (does NOT order on call). Never claim a test is already ordered.
